...
import yuuki.machinesandtech.redstoneflux.api.IEnergyStorage;
...
public class EnergyStorage implements IEnergyStorage {
    ...
    public EnergyStorage readFromNBT(ItemStack item){
        NBTItem nbti = new NBTItem(item);

        this.energy = nbti.getInteger("Energy");
    ...
    public NBTItem writeToNBT(ItemStack item){
        NBTItem nbti = new NBTItem(item);

        if (energy < 0){
            energy = 0;
        }
        nbti.setInteger("Energy", energy);
        return nbti;
    }
